...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  if (input.is_open()) {
    ...
    while (std::getline(input, line)) {
      auto str = split(line, ',');
      courses.emplace_back(str[0], str[1], str[2]);
    }
  }
  input.close();
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  if (output.is_open()) {
    output << "Title,Number of Units,Quarter" << std::endl; // 细节，没有空格！
    std::vector<Course> new_all_courses;
    for (auto [title, number_of_units, quarter] : all_courses) {
      if (quarter != "null") {
        output << title << "," << number_of_units << "," << quarter << std::endl;
      } else {
        new_all_courses.push_back({title, number_of_units, quarter});
      }
    }
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  ...
  if (output.is_open()) {
    ...
    std::vector<Course> new_all_unlisted_courses;
    for (auto [title, number_of_units, quarter] : unlisted_courses) {
      output << title << "," << number_of_units << "," << quarter << std::endl;
    }
  }
  output.close();
